...
pub fn qoffset_from_cube(offset: i32, h: Hex) -> OffsetCoord {
    let mut col: i32 = h.q;
    let mut row: i32 = h.r + (h.q + offset * (h.q & 1)) / 2 as i32;
...
pub fn qoffset_to_cube(offset: i32, h: OffsetCoord) -> Hex {
    let mut q: i32 = h.col;
    let mut r: i32 = h.row - (h.col + offset * (h.col & 1)) / 2 as i32;
    let mut s: i32 = -q - r;
    if offset != EVEN && offset != ODD {
        panic!("offset must be EVEN (+1) or ODD (-1)");
    }
...
pub fn roffset_from_cube(offset: i32, h: Hex) -> OffsetCoord {
    let mut col: i32 = h.q + (h.r + offset * (h.r & 1)) / 2 as i32;
    let mut row: i32 = h.r;
    if offset != EVEN && offset != ODD {
        panic!("offset must be EVEN (+1) or ODD (-1)");
    }
...
pub fn qdoubled_to_cube(h: DoubledCoord) -> Hex {
    let mut q: i32 = h.col;
    let mut r: i32 = (h.row - h.col) / 2 as i32;
    let mut s: i32 = -q - r;
    return Hex { q: q, r: r, s: s };
}
...
pub fn rdoubled_to_cube(h: DoubledCoord) -> Hex {
    let mut q: i32 = (h.col - h.row) / 2 as i32;
    let mut r: i32 = h.row;
    let mut s: i32 = -q - r;
    return Hex { q: q, r: r, s: s };
}
